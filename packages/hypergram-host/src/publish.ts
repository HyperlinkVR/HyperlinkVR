import { PostSchema, ProfilePictureSchema, type HostManifest } from "@hyperlinkvr/hypergram-schemas/v1";
...
export const load_site_state = async (store: SiteStore, manifest: HostManifest): Promise<SiteState> => {
    ...
    return { manifest, posts, pictures };
};
...
export const publish_site = async (store: SiteStore, state: SiteState) => {
    const files = generate_site(state);
