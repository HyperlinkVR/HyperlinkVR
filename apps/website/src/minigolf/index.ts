...
hyperlinkvr.on_ready(async () => {
    ...
    await new h.WorldEnvBuilder()
        .set_sky(
            new h.WorldSkyBuilder()
                ...
                .set_sun_size(2)
                .set_sun_glow(10)
                .set_light_intensity(1.5)
                .set_light_distance(120)
                .set_sky_light_intensity(0.3)
                .build()
        )
        .set_fog(
    ...
    // compute hole pars from markers now loaded
    compute_hole_pars();
    ...
    const fire_animation = await new h.AnimationBuilder()
        .named("fire_anim") // TODO: the need to name animations when the api is on the handle anyway is a little odd
        .add_track(
