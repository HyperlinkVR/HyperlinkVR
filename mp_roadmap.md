...
9. [~] **Session clock sync [S].** So tweens and animations can start at the same moment everywhere. **Done:** a shared session clock (`session_clock.ts`) — host is the authority (offset 0), a client estimates its offset NTP-style over a `clock` channel (`ClockSync`), keeping the lowest-RTT sample. Same `stamp` (host, at broadcast) + `session_to_local` (client, at apply) pattern is wired for **animation play/restart**, **transform tweens** (`tween_started_at` on modify), and **client-forwarded report timestamps** (converted to session time in `MultiplayerSync`) — so all start at the correct *phase*. **Left:** seeks (dynamic seek is really physics — folds into Phase D), and passive time-driven effects (VFX time-driven passes, material scroll) that read raw `performance.now()`.
...
13. [~] **Late join (snapshot, not replay) [M].** A newcomer *pulls* a snapshot of current world state from the host (clear local → request → hydrate, so nothing races), and holds a loading screen until it's hydrated. Not a command-log replay: replaying timing-dependent commands (a tween that started 10s ago) is the fragility to avoid — a snapshot captures `tween X 40% through, started at session-time T` instead. Assets round-trip via `serialise_assets` (AssetRef → url) on the host and `adopt_assets` on the client. **Done: objects** (with live position/rotation sampled from physics, stored scale preserved), **HUD, VFX, world-env** (bridged out of its React-state provider via `world_env_registry`). **Left: timing-sensitive state** (active tweens/seeks/animations with their current progress + start-time) so a late joiner catches a mid-flight animation — rides on clock sync (#9). This same serialization feeds `net.state` / host migration (#28).
...
18. [ ] **Games [S/M].** *(No longer a "port" — presence is gone, so every `max_players > 1` world is already shared; clubhouse/minigolf run shared as-is.)* Two things: **(a)** shake down an existing world under host-authoritative shared execution (surface solo assumptions, double-firing object reports, per-client leaks), and **(b)** build one genuinely player-aware world — a shared counter/score keyed by `e.player`, using `players.on_spawn`/`on_leave` — to prove #14 + #16 end to end. "Runs shared" ≠ "is multiplayer".
...
**Triggers & anticheat (resolved).** A trigger splits into a *cosmetic* part (muzzle flash/sound — wants to be instant and local) and an *authoritative* part (damage/ammo/score — must be host). These aren't a mode choice: a client-side trigger is **already sandboxed to cosmetics by construction**, because any command it produces is dropped by the bus (only the host authors shared state). So local triggers need no permission system — the command-bus authority *is* the permission. Authoritative effects run host-side off a report the host is free to reject.

Anticheat is broader than triggers and is **cleanly additive later** (the architectural half — host-owned state, non-host writes rejected — is already done). `from` proves *who* sent a report, not that it *happened*, so client-asserted events are forgeable. Trust the host as the instance authority (P2P; a cheating host needs a real server, out of scope) and tier event provenance per interaction: **assert** (trust, cosmetic/low-stakes, what we have) → **validate** (host checks reach/cooldown/re-raycast before acting) → **derive** (host detects the event itself from the synced avatar — this is why remote avatar colliders #25 are the anticheat cornerstone). Don't build a global anticheat or an `set_authority` SDK: keep authority implicit (host for state, owner for physics), concentrate the netcode in **prefabs** (a `button` bakes in local-press + report + host reach-check), and add `set_ownership` (#24) as the one physics override. Pose is the irreducible trusted-ish input (sanity-limit speed/teleport only).
